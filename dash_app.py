...
# cursor = conn.cursor()
# query = "SELECT * FROM all_weather_data"
# chunk_size = 10000  # Number of rows to load at a time
# data = pd.DataFrame()  # Empty DataFrame to hold the data
...
def update_map(country_codes):
    ...
    # Filter the data based on selected country codes
    if country_codes is None:
        query = "SELECT * FROM all_weather_data"
    else:
        query = f"SELECT * FROM all_weather_data WHERE country_code IN {tuple(country_codes)}"

    cursor.execute(query)
    rows = cursor.fetchmany(
        10000
    )  # Fetch a chunk of rows (adjust the chunk size as needed)
    data = pd.DataFrame(
        rows, columns=[description[0] for description in cursor.description]
    )
